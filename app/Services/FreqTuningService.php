...
class FreqTuningController extends Controller
{
    public function generateScript()
    {
        // Load input files
        $inputFile = storage_path('app/public/freq_tuning_input.xlsx');
        
        $spreadsheet = IOFactory::load($inputFile);
        $inputSheet = $spreadsheet->getSheetByName('input');
        $paramsSheet = $spreadsheet->getSheetByName('parameters');
        ...
        foreach ($eNBs as $eNB) {
            ...
            $filePath = storage_path("app/public/{$eNB}_freq_tuning_script_v2.mos");
        ...
        return response()->json(['message' => 'Scripts generated successfully']);
    }
    ...
    private function writeFreqTuning($file, $row)
    {
        ...
        fwrite($file, "eNB: " . $row['A'] . "\n");
        fwrite($file, "New Frequency: " . $row['B'] . "\n");
        // Add more details as per the Python logic
    }
